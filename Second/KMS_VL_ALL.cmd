@echo off
title KMS_VL_ALL Activation - Cho phien ban Volume (VOLUME ONLY) - by Nguyen Anh Duc - https://redpervn.ga
set /a _Debug=0
::==========================================
...
if %_WinBuild% GEQ 9600 (
  ...
  echo He dieu hanh hien tai: Windows 8.1 hoac hon
  goto :Win8.1AndLater
) else if %_WinBuild% GEQ 2600 (
  echo He dieu hanh hien tai: Windows 8 hoac thap hon
  goto :Win8AndEarlier
) else (
  echo KMS_VL_ALL khong ho tro he dieu hanh nay :(( sad.
  echo.
  echo 5s nua la se thoat...
...
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name /value %_Nul_2% | findstr /i "Windows" %_Nul_1% || (
  echo.
  echo Khong phat hien giay phep phien ban Windows Volume.
  if %_OfficeVL% EQU 0 (
    exit /b
  )
)
...
wmic path %_MicrosoftProduct% where (Description like '%%KMSCLIENT%%') get Name %_Nul_1_2% || (
  if %_WinBuild% LSS 9200 (
    echo.
    echo Khong phat hien giay phep phien ban Office 2010 Volume hoac hon :((.
    exit /b
  ) else (
    echo.
    echo Khong phat hien giay phep phien ban Office 2010 Volume hoac hon :((.
